const {join,normalize, dirname} = require('path'),
      {readdirSync, statSync, mkdirSync, existsSync, utimesSync, constants, chmodSync, fs} = require('fs');
...
module.exports.buildFileStat = (realPath,filePath) => {
    ...
    return {filePath, isDir, isFile, perms, bytes, createdTime, modifiedTime}
}
/**
...
module.exports.walkDirGen = async function* walkDirGen(basePath,fp) {
    ...
    for (const file of files) {
        const {filePath, isDir, isFile, perms, bytes} = module.exports.buildFileStat(join(basePath, fp),file)
        if (isFile) {
